...
export function useControlled<T = unknown>({
  ...
  name,
  state = 'value'
}: UseControlledProps<T>): [T, (newValue: T | undefined) => void] {
  ...
  if (process.env.NODE_ENV !== 'production') {
    useEffect(() => {
      if (isControlled !== (controlled !== undefined)) {
        console.error(
          [
            `UI-KIT: A component is changing the ${
              isControlled ? '' : 'un'
    ...
    useEffect(() => {
      if (!isControlled && defaultValue !== defaultProp) {
        console.error(
          [
            `UI-KIT: A component is changing the default ${state} state of an uncontrolled ${name} after being initialized. ` +
