...
import { TxButton, TxGroupButton } from "example-components/substrate/TxButton";
...
function Main(props: any) {
  ...
  const [interxType, setInterxType] = useState<any>("EXTRINSIC");
  ...
  const getApiType = (api: any, interxType: any) => {
    if (interxType === "QUERY") {
      return api.query;
  ...
  const onInterxTypeChange = (ev: any, data: any) => {
    setInterxType(data.value);
    // clear the formState
    setFormState(initFormState);
  };

  const getOptionalMsg = (interxType: any) =>
    interxType === "RPC"
      ? "Optional Parameter"
...
export default function Interactor(props: any) {
  ...
  return apiState === "READY" && api.tx ? <Main {...props} /> : null;
}
